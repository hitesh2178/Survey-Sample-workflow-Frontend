...
const SurveyResponse = () => {
  ...
  const transformLocalStorageData = (localStorageData) => {
    ...
    return localStorageData.answers.map((answer) => {
      // Handle the new JSON structure with statements
      const statements = answer.statements || [];

      // Find the selected statement based on rating (assuming highest rating is selected)
      const selectedStatement =
        statements.find((stmt) => stmt.rating === 4) ||
        statements.find((stmt) => stmt.rating === 3) ||
        statements.find((stmt) => stmt.rating === 2) ||
        statements.find((stmt) => stmt.rating === 1) ||
        statements[0];

      const selectedIndex = statements.findIndex(
        (stmt) => stmt === selectedStatement
      );
      ...
      const baseResponse = {
        ...
        selectedAnswer: selectedStatement ? selectedStatement.statement : "",
        options: statements.map((stmt) => stmt.statement),
        // Set option weights from the statements ratings
        option1: statements[0]?.rating || 1,
        option2: statements[1]?.rating || 1,
        option3: statements[2]?.rating || 1,
        option4: statements[3]?.rating || 1,
      };
  ...
  const loadFromLocalStorage = () => {
    try {
      ...
      if (storedData) {
        ...
        // Check if data has the new structure with statements
        const hasStatementsStructure = parsedData.answers?.[0]?.statements;
        ...
        if (hasStatementsStructure) {
          // Handle new JSON structure with statements
        ...
        } else if (hasCalculatedValues) {
          // Use existing calculated values if available (legacy structure)
          transformedData = parsedData.answers.map((answer) => ({
            ...
            options: answer.options
              ? answer.options.map((opt) => opt.text)
              : [],
          ...
          // Calculate values for legacy data without statements
          transformedData = parsedData.answers.map((answer) => {
            const baseResponse = {
              id: `${parsedData.respondent_id}_${answer.question_code}`,
              respondentId: parsedData.respondent_id.toString(),
      ...
      console.error(
        "Error loading survey response data from localStorage:",
        error
      );
      setResponses([]);
    }
          ...
          <div className="survey-response-header">
            ...
            <p className="survey-response-subtitle">
              ...
              >
                ...
                {Math.min(endIndex, responses.length)} of {responses.length}{" "}
                responses
              </span>
            </p>
          </div>
          ...
          {responses.length === 0 ? (
            <div className="empty-state">
              ...
              <p className="empty-state-text">
                No survey data found in localStorage
              </p>
            </div>
          ) : (
            <>
              ...
              >
                ...
                >
                  <label style={{ color: "#6b7280", fontSize: "13px" }}>
                    Records per page:
                  </label>
                  <select
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid #d1d5db",
                      backgroundColor:
                        currentPage === totalPages ? "#f9fafb" : "#fff",
